...
impl AuditLogger {
    ...
    /// 创建带配置的审计日志器
    pub fn with_config(log_file: PathBuf, enabled: bool, max_size: u64, retention_days: u32) -> Self {
        if let Some(parent) = log_file.parent() {
            let _ = fs::create_dir_all(parent);
        }
    ...
    pub fn export(&self, format: &str, output_path: &PathBuf) -> Result<()> {
        ...
        match format {
            "json" => {
                // JSON 数组格式
                let entries = self.query(&LogQuery { limit: 10000, ..Default::default() })?;
            ...
            "csv" => {
                ...
                writeln!(output, "timestamp,command,arguments,result,files_affected,duration_ms,user,cwd")?;
    ...
    pub fn get_stats(&self) -> Result<AuditStats> {
        ...
        for line in reader.lines() {
            ...
            *stats.command_counts.entry(entry.command.clone()).or_insert(0) += 1;
    ...
    fn maybe_rotate(&self) -> Result<()> {
        ...
        if let Ok(metadata) = fs::metadata(&self.log_file) {
            if metadata.len() > self.max_size {
                ...
                tracing::info!("日志已轮转：{} -> {}", self.log_file.display(), rotated_path);
            }
        }
...
mod tests {
    ...
    fn test_audit_logger_log_and_query() {
        ...
        let log_file = temp_dir.join(format!("audit_test_{}.log", SystemTime::now().duration_since(SystemTime::UNIX_EPOCH).unwrap().as_millis()));
        ...
        // 查询日志
        let query = LogQuery { limit: 10, ..Default::default() };
    ...
    fn test_audit_stats() {
        ...
        let temp_dir = std::env::temp_dir();
        let log_file = temp_dir.join(format!("audit_stats_{}.log", SystemTime::now().duration_since(SystemTime::UNIX_EPOCH).unwrap().as_millis()));
