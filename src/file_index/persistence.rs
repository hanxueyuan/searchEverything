...
use crate::file_index::trie::{TrieIndex, FileRecord, IndexStats};
...
impl IndexPersistence {
    ...
    pub fn save(&mut self, index: &TrieIndex) -> Result<()> {
        ...
        let file = File::create(&self.index_path)
            .with_context(|| format!("Failed to create index file: {}", self.index_path.display()))?;
    ...
    pub fn load(&self) -> Result<Option<TrieIndex>> {
        ...
        tracing::info!("索引已从 {} 加载，包含 {} 个文件", 
            self.index_path.display(), index.len());
...
mod tests {
    ...
    fn test_save_and_load() {
        ...
        // 加载
        let loaded_index = persistence.load()
