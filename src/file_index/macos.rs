...
/// macOS 索引构建器
pub struct MacOsIndexBuilder {
    /// 是否使用 FSEvents
    use_fsevents: bool,
}

impl MacOsIndexBuilder {
    pub fn new() -> Self {
        Self {
            use_fsevents: cfg!(target_os = "macos"),
        }
    }
...
impl IndexBuilder for MacOsIndexBuilder {
    ...
    fn supports_realtime(&self) -> bool {
        cfg!(target_os = "macos") && self.use_fsevents
    }
}

impl Default for MacOsIndexBuilder {
    fn default() -> Self {
        Self::new()
    }
}
...
fn should_exclude(path: &str, exclude_paths: &[String]) -> bool {
    ...
    // macOS 默认排除
    let default_excludes = ["/System", "/Library", "/Applications"];
