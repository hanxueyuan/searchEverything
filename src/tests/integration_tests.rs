...
use crate::file_index::trie::{TrieIndex, FileRecord};
...
fn test_index_manager_with_persistence() {
    ...
    assert!(manager.get_index().is_none());
    assert!(manager.supports_realtime() == false || manager.supports_realtime() == true);
}
...
fn test_index_config_serialization() {
    ...
    // 反序列化
    let deserialized: IndexConfig = serde_yaml::from_str(&serialized).expect("Failed to deserialize");
...
fn test_search_scenario_code_files() {
    ...
    // 添加代码文件
    index.add(create_test_record("main.rs", "/project/src/main.rs", 2048, false));
    index.add(create_test_record("lib.rs", "/project/src/lib.rs", 1536, false));
    index.add(create_test_record("utils.rs", "/project/src/utils.rs", 1024, false));
    index.add(create_test_record("config.yaml", "/project/config.yaml", 512, false));
    index.add(create_test_record("Cargo.toml", "/project/Cargo.toml", 256, false));
...
fn test_search_scenario_documents() {
    ...
    // 添加文档文件
    index.add(create_test_record("report.pdf", "/docs/report.pdf", 102400, false));
    index.add(create_test_record("notes.txt", "/docs/notes.txt", 2048, false));
    index.add(create_test_record("readme.md", "/docs/readme.md", 4096, false));
    index.add(create_test_record("spec.docx", "/docs/spec.docx", 51200, false));
...
fn test_special_characters_in_names() {
    ...
    index.add(create_test_record("file-with-dash.txt", "/home/file-with-dash.txt", 1024, false));
    index.add(create_test_record("file_with_underscore.txt", "/home/file_with_underscore.txt", 1024, false));
    index.add(create_test_record("file with space.txt", "/home/file with space.txt", 1024, false));
    index.add(create_test_record("文件.txt", "/home/文件.txt", 1024, false));
...
fn test_deep_directory_structure() {
    ...
    // 创建深层目录结构
    index.add(create_test_record("deep.txt", "/a/b/c/d/e/f/g/deep.txt", 1024, false));
    index.add(create_test_record("shallow.txt", "/shallow.txt", 1024, false));
...
fn test_duplicate_handling() {
    ...
    // 添加相同路径的文件
    index.add(create_test_record("file.txt", "/home/file.txt", 1024, false));
    index.add(create_test_record("file.txt", "/home/file.txt", 2048, false)); // 不同大小
    index.add(create_test_record("file.txt", "/home/file.txt", 4096, false)); // 再次更新
...
fn test_index_build_performance() {
    ...
    // 构建应该在合理时间内完成（< 5 秒）
    assert!(build_time.as_secs() < 5, "Index build took too long: {:?}", build_time);
}
...
fn test_search_performance_various_patterns() {
    ...
    for (pattern, search_type) in patterns {
        ...
        println!("{} search '{}' returned {} results in {:?}", 
                 search_type, pattern, results.len(), elapsed);
        
        // 搜索应该在合理时间内完成（正则搜索允许更长时间）
        let max_time = if search_type == "regex" { 200 } else { 100 };
        assert!(elapsed.as_millis() < max_time, 
                "{} search took too long: {:?}", search_type, elapsed);
    }
}
