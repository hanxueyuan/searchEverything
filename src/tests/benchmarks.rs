...
mod benchmarks {
    ...
    fn benchmark_prefix_search() {
        ...
        println!("1000 次前缀搜索：{:?} (平均 {:?} 每次)", duration, duration / 1000);
        // Debug 模式下性能较低，release 模式下会快很多
        assert!(duration.as_secs() < 10, "1000 次搜索应该小于 10 秒 (debug 模式)");
    }
    ...
    fn benchmark_glob_search() {
        ...
        println!("1000 次通配符搜索：{:?} (平均 {:?} 每次)", duration, duration / 1000);
    }
    ...
    fn benchmark_fuzzy_search() {
        ...
        println!("100 次模糊搜索：{:?} (平均 {:?} 每次)", duration, duration / 100);
    }
    ...
    fn benchmark_add_remove() {
        ...
        println!("1000 次移除操作：{:?} (平均 {:?} 每次)", duration, duration / 1000);
