...
fn test_trie_prefix_search() {
    ...
    index.add(create_test_record("document.txt", "/home/document.txt", 4096));
...
fn test_trie_fuzzy_search() {
    ...
    index.add(create_test_record("configuration.yaml", "/home/configuration.yaml", 1024));
...
fn test_trie_add_remove() {
    ...
    assert!(index.get_by_path(&PathBuf::from("/home/file1.txt")).is_none());
    assert!(index.get_by_path(&PathBuf::from("/home/file2.txt")).is_some());
}
