...
impl SkillTester {
    ...
    fn run_single_test(&self, test_case: &SkillTestCase) -> SkillTestResult {
        ...
        let error = if !passed {
            if test_case.should_match {
                Some(format!(
                    "期望命令：{}, 实际命令：{:?}",
                    test_case.expected_command, matched_command
                ))
            } else {
                Some(format!(
                    "不应该匹配任何命令，但匹配到了：{:?}",
    ...
    fn match_command(&self, input: &str) -> Option<String> {
        // 复制命令 - 需要在 info 之前匹配
        if input.contains("复制") || input.contains("拷贝") {
            return Some("copy".to_string());
        }

        // 移动命令 - 需要在 info 之前匹配
        if input.contains("移动")
            || input.contains("剪切")
            || (input.contains("移到") && !input.contains("信息"))
        {
            return Some("move".to_string());
        }

        // 删除命令 - 需要在 info 之前匹配
        if input.contains("删除")
            || input.contains("移除")
                && (input.contains("文件") || input.contains("夹") || input.contains("目录"))
            || input.contains("不要.*了")
            || input.contains("清理")
        {
        ...
        // 搜索命令
        if input.contains("搜索")
            || input.contains("查找")
            || input.contains("有没有")
            || input.contains("在哪里")
            || input.contains("在哪个目录")
        {
            return Some("search".to_string());
        }

        // 信息命令
        if input.contains("信息") || input.contains("多大") || input.contains("什么时候修改")
        {
        ...
        if input.contains("打开")
            || input.contains("读取")
            || input.contains("查看内容")
            || input.contains("显示.*内容")
            || input.contains("里面是什么")
        {
            return Some("cat".to_string());
        }

        // 配置命令
        if input.contains("配置") {
            return Some("config".to_string());
        }

        // 索引命令
        if input.contains("索引") {
